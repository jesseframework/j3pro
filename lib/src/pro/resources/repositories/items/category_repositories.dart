      ...
      if (isSchedulerEnable != null) {
        ...
        if (isSchedulerEnable.startDateTime.isBefore(DateTime.now())) {
          if (isSchedulerEnable.enableJob == true) {
            ...
            _log.finest("Category jobs start date is $startDate ");
            final Response response = await api.getAllCategory();
            _log.finest("Checking server resopnses for category ");
            ...
            if (response.isSuccessful && map['success']) {
              ...
              for (var item in items) {
                if (isStopped) break;
                await categoryDao.createOrUpdateCateory(item);
              }
